...
const playGame = () => {
	...
	const playRound = student => {
		...
		if (round === rounds) { // show different message in btn-next if last round
			btnNextEl.innerText = "See result";
		}
	}

	btnNextEl.addEventListener('click', e => {
		if (btnNextEl.innerText === "Next mate") {
			playRound(students[round]);
		} else if(btnNextEl.innerText === "See result") {
